...
@Component({
  selector: 'app-import-file-kich-co',
  templateUrl: './import-file-kich-co.component.html',
  styleUrls: ['./import-file-kich-co.component.scss']
})
export class ImportFileSizeComponent implements OnInit {
  typeImport = 0;
  isShowImport = false;
  nameFile: any;
  sizeFile: any;
  ...
  constructor(
    private matRef: MatDialogRef<ImportFileSizeComponent>,
  ...
  downloadSampleFile() {
    this.sizeService.exportDataTemplate().subscribe(response => {
  ...
  importFile(): void {
    ...
    this.sizeService.onUpload(this.formDatas).subscribe(
      (response: string) => {
