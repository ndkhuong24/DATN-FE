...
@Component({
  selector: 'app-import-file-danh-muc',
  templateUrl: './import-file-danh-muc.component.html',
  styleUrls: ['./import-file-danh-muc.component.scss']
})

export class ImportFileCategoryComponent implements OnInit {
  typeImport = 0;
  isShowImport = false;
  nameFile: any;
  sizeFile: any;
  ...
  constructor(
    private matRef: MatDialogRef<ImportFileCategoryComponent>,
  ...
  downloadSampleFile() {
    this.categoryService.exportDataTemplate().subscribe(response => {
      ...
      // Set link attributes
      link.href = url;
      link.download = `DS_DanhMuc_Template.xlsx`;
  ...
  importFile(): void {
    ...
    this.categoryService.onUpload(this.formDatas).subscribe(
      (response: string) => {
