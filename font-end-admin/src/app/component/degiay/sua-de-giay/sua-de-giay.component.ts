...
export class SuaDeGiayComponent implements OnInit {
  ...
  validateSoleHeight() {
    this.validSoleHeight = CommonFunction.validateInput(
      this.data.soleHeight,
      250,
      null
    );
  }
  ...
  clickUpdate(id: number) {
    ...
    if (
      !this.validSoleHeight.done ||
      !this.validSoleMaterial.done ||
      !this.validDescription.done
    ) {
      return;
    }
    Swal.fire({
